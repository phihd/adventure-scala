...
object AdventureGUI extends SimpleSwingApplication with O1SwingDefaults {
  ...
  def top = new MainFrame {
    ...
    this.contents = new GridBagPanel { 
      ...
      //layout += new Label("Army:")     -> new Constraints(0, 0, 1, 1, 0, 1, NorthWest.id, Fill.None.id, new Insets(8, 5, 5, 5), 0, 0)
      ...
      layout += moneyCounter           -> new Constraints(0, 4, 2, 1, 0, 0, NorthWest.id, Fill.None.id, new Insets(8, 5, 5, 5), 0, 0)
    ...
    this.minimumSize = new Dimension(1280, 660)
    this.pack()
    ...
    def updateInfo(info: String) = {
      ...
      this.turnCounter.text = (if (player.playerName == "") "Anonymous" else player.playerName) + "'s army: " +
      player.Stat.HP + "/" + player.maxHP + " " + " health " + ", Attack: " + player.Stat.Attack + ", Defense: " + player.Stat.Defense + ", Speed: " + player.Stat.Speed + ", Critical rate: " + player.Stat.Critical + "%"
      this.moneyCounter.text = "Money: " + player.money    
    }
