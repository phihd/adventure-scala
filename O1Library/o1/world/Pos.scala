...
final case class Pos(val x: Double, val y: Double) { 
  ...
  /** Returns a `Pos` whose two coordinates are obtained by multiplying this one’s `x` and `y` by the given factor. 
    * For instance, (2,-3) multiplied by 10 is (20,-30). The expressions `myPos.divide(n)` and `myPos / n` are equivalent. */
  ...
  @inline def withXYX(computeNew: (Double, Double) => (Double, Double)): Pos = Pos(computeNew(this.x, this.y))
