...
/** Each instance of this class represents movement in the context of a two-dimensional plane.
  * A velocity is a combination of a [[Direction]] of movement with a speed (a `Double`). 
  *
  ...
  * 
  *  - construct a velocity from a direction and a speed, as in 
  *    `Velocity(Direction.Left, 50)`;
  *  - construct a direction from `dx` and `dy`, as in 
  *    `Velocity(-100, 50)`; or
  *  - determine the velocity needed to go from one [[Pos]] to
  *    another in a single unit of time, as in 
  *    `Velocity.between(pos1, pos2)`. 
  *  
  * Many of the methods of a `Velocity` object also create and return new velocities.
  * 
...
final case class Velocity(val direction: Direction, val speed: Double) {
  ...
  /** Returns a velocity whose `speed` equals this velocity’s speed multiplied by the given number. 
    * The direction is the same. */ 
  def *(multiplier: Double): Velocity = this.copy(speed = this.speed * multiplier)

  /** Returns a velocity whose `speed` equals this velocity’s speed divided by the given number. 
    * The direction is the same. */ 
  ...
  /** Returns a velocity whose `speed` equals this velocity’s speed plus the given number. 
    * The direction is the same. */ 
  def faster(addition: Double): Velocity  = Velocity(this.direction, this.speed + addition)
  
  /** Returns a velocity whose `speed` equals this velocity’s speed minus the given number. 
    * The direction is the same. */ 
  def slower(reduction: Double): Velocity = Velocity(this.direction, this.speed - reduction).noSlowerThan(0)
  ...
  /** Returns a velocity whose `speed` is as close to this one’s as possible without exceeding the given number. 
    * The direction is the same. */ 
  def noFasterThan(maxSpeed: Double): Velocity = this.copy(speed = this.speed atMost maxSpeed)
  ...
  /** Returns the `Pos` that an object moving at this velocity reaches in one unit of time.   
    * That is, adds the `dx` and `dy` components of this velocity to the given `Pos` and returns the result. */  
