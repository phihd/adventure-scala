...
/** The class `Grid` represents rectangular grids that contain elements of a particular 
  ...
  *
  * Upon creation, a `Grid` initializes itself by first calling [[initialElements]], which
...
abstract class Grid[Element: ClassTag](val width: Int, val height: Int) {
  ...
  private val contents = {
    val elems = this.initialElements
    assert(elems.size == this.size, s"The number of elements returned by initialElements (${elems.size}) did not equal width times height ($size) as required.")
  ...
  def elementAt(location: GridPos) = 
    if (this.contains(location)) this.contents(location.x)(location.y) else throw new IllegalArgumentException(s"Attempted to access a non-existent location $location of a ${width}-by-$height grid.") 
  ...
  def update(location: GridPos, newElement: Element) = {
    if (this.contains(location)) {
      ...
      throw new IllegalArgumentException(s"Attempted to update element at a non-existent location $location of a ${width}-by-$height grid.")
  }
  ...
  /** Generates the elements that initially occupy the grid. This method is automatically
    ...
    * 
    * Note that since this method is invoked to produce the content of the `Grid`, it
    * is called during initialization before the `Grid` actually has any elements as content. 
  ...
  /** Returns a list of all the neighboring elements of the indicated element. Depending on 
    ...
    *
    * @param location          the location between the neighbors
  ...
  def neighbors(middleLoc: GridPos, includeDiagonals: Boolean) = {
  ...
  def allPositions: Vector[GridPos] = (0 until this.size).map( n => GridPos(n % this.width, n / this.width) ).toVector  
    
    
  /** Returns a collection of all the elements currently in the grid. */
  def allElements: Vector[Element] = for (pos <- this.allPositions; elem <- Option(this(pos))) yield elem
