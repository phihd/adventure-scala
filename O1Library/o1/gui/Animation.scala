...
class Animation(frames: Iterable[Pic], val frameRate: Double, val terminateOnClose: Boolean, val atEnd: AtEnd) {
  ...
  private val eachFrameOnce = frames.toStream

  /** Returns the frames of this animation as a `Stream`. If [[atEnd]] is set to [[Animation 
    * Animation.Loop]], this will be a repeating, infinite stream. Otherwise, it just contains 
    * each frame once, as set by the constructor parameter. */ 
  def frameStream: Stream[Pic] =
  ...
  private class Screen extends immutable.View(initialState = this.frameStream, frameRate, terminateOnClose = this.terminateOnClose, 
                                              closeWhenDone = (this.atEnd == Close), refreshPolicy = UnlessSameReference) {
    private var isPaused = false

    def makePic(upcomingFrames: Stream[Pic]): Pic =
      upcomingFrames.headOption.getOrElse(throw NothingToDraw)

    override def onClick(upcomingFrames: Stream[Pic], position: Pos): Stream[Pic] = {
      this.close()
      upcomingFrames
    }

    override def onTick(upcomingFrames: Stream[Pic]): Stream[Pic] = {
    ...
    override def isPaused(upcomingFrames: Stream[Pic]): Boolean = this.isPaused

    override def onMouseDown(upcomingFrames: Stream[Pic], position: Pos): Stream[Pic] = {
...
object Animation {
  ...
  def generate(makeFrame: Int => Pic, numberOfFrames: Int = Int.MaxValue, frameRate: Double = 5, 
    ...
    this.show(Stream.tabulate(numberOfFrames)(makeFrame), frameRate, terminateOnClose, atEnd)
  }
