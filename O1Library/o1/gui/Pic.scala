...
import o1.gui.PicHistory.Operations.{CreationOperation, MiscellaneousOperation, TransformationOperation, ViewportOperation}
import o1.util._
import o1.world._
...
object Pic extends ShapeAPI {
  ...
  @inline def apply(pathOrURL: String, anchor: Anchor): Pic = {
    ...
    val newHistory = PicHistory(CreationOperation(methodName = "Pic", simpleDescription = pathOrURL))
  ...
  /** Displays the given [[Pic]] in a minimalistic GUI window. Calling this method repeatedly on the
    ...
    * @param backgroundColor  the color to appear behind the `Pic` in the frame (where transparent)
    * @param borderWidth      the width, in pixels, of the simple black window frame
    * @see [[hide]]
    * @see [[Pic.show]] */
 def show(pic: Pic, backgroundColor: Color, borderWidth: Int): Unit = {
    val reasonNotToShow = pic.dimensions match  {
      ...
      case pictureLargeEnoughToShow => if (View.isInTestMode) Some("Not actually showing the picture because in text-based test mode.") else None
    }
    reasonNotToShow match {
      case Some(reason) => println(reason)
      case None         => PicFrame.show(pic, backgroundColor, borderWidth)
    }
  }
  ...
  def generate(width: Int, height: Int, makeColor: (Int, Int) => Color): Pic = {
    ...
    val anchor = Center
    val newHistory = PicHistory(CreationOperation(methodName = "Pic.generate", simpleDescription = "generated pic"))
  ...
  def fromColors(width: Int, height: Int, colors: Seq[Color]): Pic = {
    if (colors == null)
      throw new IllegalArgumentException("The sequence of pixel colors cannot be null")
    val smclContent = smcl.pictures.Bitmap(width, height, colors.map(_.smclColor))
    val anchor = Center
    val newHistory = PicHistory(CreationOperation(methodName = "Pic.fromColors", simpleDescription = "composed pic"))
    Pic(smclContent, anchor, newHistory)
  }
...
final class Pic private[gui](private[gui] val smclPic: SMCLPicture, val anchor: Anchor, private[o1] val historyObject: PicHistory) extends HasAnchor {
  ...
  lazy val history: List[String] = this.historyObject.methodNameList
  ...
  val historyLength: Int = this.historyObject.maximumLength


  /** Sets a new limit to the history length on the `Pic`.
    * @param a new [[historyLength]] (>= 0) */
  def withHistoryLength(maxLength: Int): Pic = {
    val newHistory = this.historyObject.setHistoryLength(maxLength)
    Pic(this.smclPic, this.anchor, newHistory)
  }
  ...
  @inline override def toString: String = {
    ...
    s ++= this.historyObject.creationOperation.simpleDescription
    if (this.historyObject.containsTransformations) {
      s ++= " (transformed)"
    }
    s.toString()
  }
  ...
  @inline def freeze: Pic = {
    val newHistory = this.historyObject.add(MiscellaneousOperation(methodName = "freeze"))
    Pic(this.toSMCLBitmap, this.anchor, newHistory)
  }
  ...
  @inline def isBitmap: Boolean =
    this.smclPic.elements.length == 1 && this.smclPic.elements.head.isBitmap
  ...
  /** Returns the colors of the pixels in this `Pic` that are “near” a given [[Pos]]. A pixel
    * counts as being near if its located at a `Pos` whose distance from the given `Pos` is less
  ...
  def pixelsNear(pos: Pos, range: Double): Seq[Color] = {
    ...
    val possNear = for {
      y <- pos.y - maxDelta to pos.y + maxDelta by 1.0
      x <- pos.x - maxDelta to pos.x + maxDelta by 1.0
      candidate = Pos(x, y)
  ...
  @inline def anchorAt(anchor: Anchor): Pic = {
    val newHistory = this.historyObject.add(MiscellaneousOperation(methodName = "anchorAt"))
    Pic(this.smclPic, anchor, newHistory)
  }
  ...
  @inline private[gui] def setViewport(viewport: Viewport): Pic = {
    val newHistory = this.historyObject.add(ViewportOperation(methodName = "setViewport"))
    setViewportInternal(viewport, newHistory)
  }

  @inline private[gui] def setViewportWithoutHistory(viewport: Viewport): Pic = {
    setViewportInternal(viewport, this.historyObject)
  }

  @inline private def setViewportInternal(viewport: Viewport, newHistory: PicHistory): Pic = {
    val smclContent =
      if (viewport == null || viewport.isUndefined)
        this.smclPic.removeViewport
      else
        this.smclPic.setViewport(viewport.toSMCLViewport.get)
  ...
  @inline private[gui] def removeViewport: Pic = {
    val newHistory = this.historyObject.add(ViewportOperation(methodName = "removeViewport"))
    removeViewportInternal(viewport, newHistory)
  }

  @inline private[gui] def removeViewportWithoutHistory(viewport: Viewport): Pic =
    removeViewportInternal(viewport, this.historyObject)
  ...
  @inline def transformColors(transformer: Color => Color): Pic = {
    ...
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "transformColors"))
    Pic(smclContent, this.anchor, newHistory)
  }
  ...
  @inline def transformXY(newColorAt: (Int, Int) => Color): Pic = {
    ...
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "transformXY"))
    Pic(smclContent, this.anchor, newHistory)
  }
  ...
  /** Returns a partial `Pic` of a rectangular area within this `Pic`.
    * @param  the cropping frame within this `Pic`
  ...
  @inline def crop(boundary: Bounds): Pic = {
    ...
    val smclContentWithViewport =
      if (this.smclPic.hasViewport)
        smclContent.setViewportToContentBoundary(name = None)
      else
        smclContent
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "crop"))
    Pic(smclContentWithViewport, this.anchor, newHistory)
  }
  ...
  @inline def scaleBy(factor: Double): Pic = {
    ...
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "scaleBy"))
    Pic(smclContent, this.anchor, newHistory)
  }
  ...
  @inline def scaleTo(targetWidth: Double, targetHeight: Double): Pic = {
    ...
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "scaleTo"))
    Pic(smclContent, this.anchor, newHistory)
  }
  ...
  @inline def flipHorizontal: Pic = {
    ...
    val smclContentWithViewport =
      if (this.smclPic.hasViewport)
        smclContent.setViewport(this.smclPic.viewport.get)
      else
        smclContent
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "flipHorizontal"))
    Pic(smclContentWithViewport, this.anchor, newHistory)
  }
  ...
  @inline def flipVertical: Pic = {
    ...
    val smclContentWithViewport =
      if (this.smclPic.hasViewport)
        ...
        smclContent
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "flipVertical"))
    Pic(smclContentWithViewport, this.anchor, newHistory)
  }
  ...
  @inline def flipDiagonal: Pic = {
    ...
    val smclContentWithViewport =
      if (this.smclPic.hasViewport)
        ...
        smclContent
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "flipDiagonal"))
    Pic(smclContentWithViewport, this.anchor, newHistory)
  }
  ...
  @inline def clockwise(degrees: Double = 90.0): Pic = {
    ...
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "clockwise"))
    Pic(Some(smclContent), this.anchor, newHistory)
  }
  ...
  @inline def counterclockwise(degrees: Double = 90.0): Pic = {
    ...
    val newHistory = this.historyObject.add(TransformationOperation(methodName = "counterclockwise"))
    Pic(Some(smclContent), this.anchor, newHistory)
  }
  ...
  @inline def shiftRight(offset: Double): Pic = {
    ...
    val smclContentWithViewport =
      if (this.smclPic.hasViewport)
        ...
        smclContent
    Pic(smclContentWithViewport, this.anchor, this.historyObject)
  }
  ...
  /** Shifts each pixel within the `Pic` downwards by the given amount. Retains the
    ...
    * @return a new `Pic` (a bitmap) that contains a shifted version of the original */
  @inline def shiftDownwards(offset: Double): Pic = {
  ...
  /** Shifts each pixel within the `Pic` upwards by the given amount. Retains the
    ...
    * @return a new `Pic` (a bitmap) that contains a shifted version of the original */
  @inline def shiftUpwards(offset: Double): Pic = {
    val normalizedOffset = offset % this.height
    if (normalizedOffset == 0) return this
    val actualOffset = if (normalizedOffset > 0) normalizedOffset else (this.height + normalizedOffset) % this.height
  ...
  /** Combines this `Pic` and the given one so that this `Pic` appears in front of the other
    ...
    * @return the combined `Pic`
    * @see [[o1.gui.Pic.against(background:o1\.gui\.Pic,at:o1\.gui\.Pos)* against]],
    *      which does the same but leaves out any “hanging” parts of the foreground image */
  ...
  /** Combines this `Pic` and the given one so that this `Pic` appears in front of the other `Pic`
    ...
    * @return the combined `Pic`
    * @see [[o1.gui.Pic.against(background:o1\.gui\.Pic,my:o1\.gui\.Anchor,at:o1\.gui\.Pos)* against]],
    *      which does the same but leaves out any “hanging” parts of the foreground image */
  ...
  @inline private def onto(backPic: Pic, my: Anchor, at: Pos, newViewport: Viewport): Pic = {
    ...
    val newSMCLContentWithViewport =
      if (newViewport == null || newViewport.isUndefined)
        newSMCLContent.removeViewport
      else
        newSMCLContent.setViewport(newViewport.toSMCLViewport.get)

    val newAnchor = backPic.anchor
    val newHistory = historyForCombinedPicture

    Pic(newSMCLContentWithViewport, newAnchor, newHistory)
  }
  ...
  /** Combines this `Pic` and the given one so that the other `Pic` serves as a background and this
    ...
    * @return the combined `Pic`
    * @see [[o1.gui.Pic.onto(backPic:o1\.gui\.Pic,at:o1\.gui\.Pos)* onto]],
    *      which does the same but doesn’t leave out the “hanging” parts of the foreground image
    * @see [[o1.gui.Pic.place(foreground:o1\.gui\.Pic,at:o1\.gui\.Pos)* place]],
  ...
  /** Combines this `Pic` and the given one so that the other `Pic` serves as a background and this
    ...
    * @return the combined `Pic`
    * @see [[o1.gui.Pic.onto(backPic:o1\.gui\.Pic,my:o1\.gui\.Anchor,at:o1\.gui\.Pos)* onto]]
    *      which does the same but doesn’t leave out the “hanging” parts of the foreground image
    * @see [[o1.gui.Pic.place(foreground:o1\.gui\.Pic,its:o1\.gui\.Anchor,at:o1\.gui\.Pos)* place]]
    *      which does the same but switches the order of the two `Pic`s */
  ...
  /** Combines this `Pic` and the given one so that this `Pic` serves as a background and the other
    ...
    * @return the combined `Pic`
    * @see [[o1.gui.Pic.against(background:o1\.gui\.Pic,my:o1\.gui\.Anchor,at:o1\.gui\.Pos)* place]]
    *      which does the same but switches the order of the two `Pic`s */
  ...
  /** Combines this `Pic` and the given one so that this `Pic` serves as a background and the other
    ...
    * @return the combined `Pic`
    * @see [[o1.gui.Pic.against(background:o1\.gui\.Pic,at:o1\.gui\.Pos)* place]],
    *      which does the same but switches the order of the two `Pic`s */
  ...
  /** Combines this `Pic` and the given ones so that this `Pic` serves as a background and the
    ...
    * [[o1.gui.Pic.place(foreground:o1\.gui\.Pic,at:o1\.gui\.Pos)* place(Pic,Pos)]] repeatedly.
    ...
    * @return the combined `Pic` */
  @inline def place(foregroundPics: Traversable[(Pic, Pos)]): Pic = foregroundPics.foldLeft(this)( _.place(_) )
  ...
  /** Combines this `Pic` and copies of the given one so that this `Pic` serves as a background and the copies
    ...
    * `Pic` is the size of this background `Pic`. This is equivalent calling [[o1.gui.Pic.place(foreground:o1\.gui\.Pic,at:o1\.gui\.Pos)* place(Pic,Pos)]]
  ...
  @inline def rightOf(leftPic: Pic, retainAnchor: Boolean = false): Pic = {
    ...
    val newAnchor = if (retainAnchor) this.absoluteAnchor else this.anchor
    val newHistory = historyForCombinedPicture
    Pic(newSMCLContent, newAnchor, newHistory)
  }
  ...
  @inline def alternatingColumn(another: Pic, number: Int): Pic =
    if (number < 0) {
      ...
      return Pic(None, this.anchor, historyForCombinedPicture)
    } else {
  ...
  private def historyForCombinedPicture: PicHistory = {
    val creation = CreationOperation(methodName = "<combining method>", simpleDescription = "combined pic")
    this.historyObject.copy(creation, List())
  }
