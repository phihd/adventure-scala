...
package object util extends iofuncs {
  ...
  /** An alias for convenient use of `java.io.File` via `import o1.util._`. */
  ...
  /** This class extends the interface of `scala.Traversable` with convenience methods. */
  implicit class ConvenientCollection[Element](private val self: Traversable[Element]) extends AnyVal {

    /** Constructs and returns a new map from the collection by applying a key-generating function
      * as well as a value-generating function to each element of the collection. The respective
    ...
    /** Performs a given side effect at each element of the collection, then returns that element.
      * Useful for generating printouts while debugging, for instance. */
    def tap(effect: Element => Unit) = self.map { element =>
      effect(element)
      element
    }
  }
  ...
  implicit class ConvenientInt(private val value: Int) extends AnyVal {
    ...
    def isOdd: Boolean = this.value % 2 == 1
